...
public class DataHandlerRandomTest {
	...
	//clean up file before test
		@Before
		public void setUp() {
				File testfile = new File("ctest.xml");
				testfile.delete();
		}
	 ...
	 public void randomtest()  throws Throwable  {
		 ...
		 for (int iteration = 0; elapsed < TestTimeout; iteration++) {
			 try{
				...
				boolean autosaveSet=ValuesGenerator.getBoolean(.60f, random);
				...
				int endYear=ValuesGenerator.getRandomIntBetween(random, 2019, 2100);

				//random number of appointments
				int numAppts=ValuesGenerator.getRandomIntBetween(random, 0, 11);

				//setup the random firstday and lastday
				DataHandler data0 = new DataHandler("ctesdt.xml", autosaveSet);
				...
				while(numAppts > 0){
				 ...
				 int apptYear=ValuesGenerator.getRandomIntBetween(random, 2010, 2020);

				 Appt appt0 = new Appt(apptHour, apptMinute, apptDay, apptMonth, apptYear, "Birthday", "This is a birthday.", "xyz@gmail.com");
